...
function createQueue(client, config) {
    ...
    const embeddedWorld = (0, world_local_1.createEmbeddedWorld)({ dataDir: undefined, port });
    ...
    async function pollQueue(queueUrl, queuePrefix) {
        while (!shouldStop) {
            try {
                const response = await client.send(new client_sqs_1.ReceiveMessageCommand({
                    QueueUrl: queueUrl,
                ...
                for (const message of response.Messages) {
                    ...
                    try {
                        ...
                        const payload = world_1.QueuePayloadSchema.parse(transport.deserialize(body.data));
                        ...
                        const handler = createQueueHandler(queuePrefix, async (msg, meta) => {
                            // The embedded world will handle the actual workflow/step execution
                        });
                        // Make an HTTP request to the embedded world
                        const request = new Request(`http://localhost:${port || 3000}/.well-known/workflow/v1/${queuePrefix === '__wkf_workflow_' ? 'flow' : 'step'}`, {
                            method: 'POST',
